import { cropReducer, InitialCropState } from '../src/components/Cropper/Reducer';

describe('cropReducer', () => {
	it('should return the initial state', () => {
		expect(cropReducer(InitialCropState, { type: 'RESET' }))
		.toEqual({
	...
	it('should handle SELECT', () => {
		expect(cropReducer(InitialCropState, { type: 'SELECT', payload: 1 }))
		.toEqual({
			cropSettings: [{ box: { x: 0, y: 0, width: 0, height: 0 } }],
			selected: 1,
		});
	...
	it('should handle ADD_CROPPER', () => {
		expect(cropReducer(InitialCropState, { type: 'ADD_CROPPER' }))
		.toEqual({
			cropSettings: [{ box: { x: 0, y: 0, width: 0, height: 0 } }, { box: { x: 0, y: 0, width: 0, height: 0 } }],
			selected: 1,
		});
	});
	...
	it('should handle CHANGE_SIZE', () => {
		// first change
		expect(cropReducer(InitialCropState, {
			type: 'CHANGE_SIZE',
			payload: {
